...
import * as Types from '../../src/typecheck/types';


const { TType, TInteger, TFloat, TChar, TBool, TTuple, TStruct, TArray, TFunction, TUnion, TAny, TUnknown, determineGeneralType } = Types;

function getSuperClass(c) {
...
describe('Typecheck Type Classes', () => {
    it('should be assignable from TUnknown', () => {
        ...
        for (const t of Object.values(Types)) {
            if (t !== TUnknown && typeof(t) === 'function' && getSuperClass(t) === TType) {
                expect(new t().isAssignableFrom(new TUnknown())).to.eql(true);
            }
        }
    });
    ...
    describe('TFunction', () => {
        it('should not be assignable from non-function', () => {
            expect(new TFunction().isAssignableFrom(new TStruct())).to.eql(false);
        });

        it('should not be assignable from different param number', () => {
            expect(new TFunction([new TBool()]).isAssignableFrom(new TFunction([new TBool(), new TBool()]))).to.eql(false);
        });
    ...
    describe('TUnion', () => {
        ...
        it('should not be assignable from a non-element', () => {
            expect(new TUnion([new TChar(), new TBool()]).isAssignableFrom([new TFloat(64)])).to.eql(false);
        });
    ...
    describe('TAny', () => {
        it('should be assignable from all types', () => {
            ...
            for (const t of Object.values(Types)) {
                if (typeof(t) === 'function' && getSuperClass(t) === TType) {
                    expect(new TAny().isAssignableFrom(new t())).to.eql(true);
                }
            }
        });
    });
    ...
    describe('determineGeneralType', () => {
        it('should handle falsy types', () => {
            expect(determineGeneralType(null, new TBool())).to.eql(new TBool());
            expect(determineGeneralType(new TBool(), null)).to.eql(new TBool());
            expect(determineGeneralType(null, null)).to.eql(null);
        });
