...
export default class AssignmentVisitor implements ITypeVisitor<bool> {
    ...
    @baseCheck
    visitOverloadedGeneric(to: TOverloadedGeneric): boolean {
        // if we are trying to assign to an overloaded generic type,
        // it means we are trying to assign to the param-less type
        const paramLess = to.getParamLessType();
        if (paramLess) return paramLess.visit(this);
        // there is no param-less type, so this shouldn't even be happening
        return false;
    }
